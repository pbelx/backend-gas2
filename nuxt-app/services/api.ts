...
class ApiService {
  ...
  private async request<T>(
    ...
    try {
      ...
      // console.log('Request config:', config); // Too verbose for general use
      // console.log('Request body:', options.body); // Too verbose, can expose sensitive data
      ...
      // console.log('API response:', data); // Too verbose for general use
  ...
  async createUser(userData: RegisterData, token: string): Promise<User> { // Added token for consistency/auth
    return this.request<User>('/users', {
      method: 'POST',
      headers: { // Assuming user creation might be protected
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify(userData),
    });
  ...
  async createOrder(orderData: CreateOrderPayload, token: string): Promise<Order> {
    ...
    return this.request<Order>('/orders', {
      ...
      headers: {
        'Authorization': `Bearer ${token}`,
        // Content-Type is set by `request` method if body is string
      },
  ...
  async cancelOrder(id: string, token: string): Promise<Order> {
    return this.request<Order>(`/orders/${id}/cancel`, {
      method: 'PUT', // Assuming cancel is a PUT to update status, adjust if it's a POST or PATCH
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
// For Nuxt 3, it's better to instantiate this in a plugin or on demand
// to leverage runtimeConfig for the baseURL.
// export const apiService = new ApiService(); // Avoid direct instantiation here for Nuxt 3
